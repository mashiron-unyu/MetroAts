...
namespace TokyuSignal {
    ...
    public partial class TokyuSignal : AssemblyPluginBase {
        public override void Tick(TimeSpan elapsed) {
            ...
            if (SignalEnable) {
                if (StandAloneMode) {
                    ...
                    panel[28] = Config.SignalSWLists[NowSignalSW] == SignalSWListStandAlone.Noset ? 1 : 0;
                    if (currentSection.CurrentSignalIndex >= 9 && currentSection.CurrentSignalIndex != 34 && currentSection.CurrentSignalIndex < 49) {
                        ...
                        if (!ATC.ATCEnable) 
                            sound[0] = Config.SignalSWLists[NowSignalSW] == SignalSWListStandAlone.ATC ? (int)AtsSoundControlInstruction.Stop : (int)AtsSoundControlInstruction.PlayLooping;
                    ...
                    if (currentSection.CurrentSignalIndex >= 9 && currentSection.CurrentSignalIndex != 34 && currentSection.CurrentSignalIndex < 49) {
                        ...
                        if (!ATC.ATCEnable)sound[0] = (corePlugin.SignalSWPos == MetroAts.SignalSWList.ATC)
        ...
        private static void UpdatePanelAndSound(IList<int> panel, IList<int> sound) {
            ...
            if (ATC.ATCEnable) panel[32] = Convert.ToInt32(ATC.ATC_Depot);
            if (ATC.ATCEnable && ATC.ATC_Noset) panel[28] = Convert.ToInt32(ATC.ATC_Noset);
            ...
            sound[2] = (int)ATC.ATC_Ding;
            sound[3] = (int)ATC.ATC_ORPBeep;
            if (ATC.ATC_SignalAnnBeep == AtsSoundControlInstruction.Play)
                sound[4] = (int)AtsSoundControlInstruction.Stop;
            sound[4] = (int)ATC.ATC_SignalAnnBeep;
            sound[19] = (int)TokyuATS.ATS_WarnBell;
            if (TokyuATS.ATSEnable) {
                sound[0] = (int)TokyuATS.ATS_EBBell;
            }
            if (ATC.ATCEnable) {
                sound[0] = (int)ATC.ATC_WarningBell;
            }
